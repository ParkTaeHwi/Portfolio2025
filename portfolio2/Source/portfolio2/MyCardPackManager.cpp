...
void AMyCardPackManager::BeginPlay()
{
	...
	if (!MyGameInstance || !CardPackClass) return;
...
void AMyCardPackManager::InitializeCardPacks()
{
	if (!CardPackClass || !MyGameInstance) return; // 제일 위로 이동!

	// 이미 데이터가 있다면 새로 만들지 않음
	if (MyGameInstance->CardPackDataList.Num() > 0)
	{
		UE_LOG(LogTemp, Warning, TEXT("CardPackDataList already exists. Skipping initialization."));
		SpawnCardPacksFromData(); // 기존 데이터로 카드팩 생성
		return;
	}
	...
	int32 TotalCardPacks = ColumnCount * RowCount;
	...
	int32 NumWhite = TotalCards - NumBlue - NumYellow;

	TArray<FString> BlueCards, YellowCards, WhiteCards;
	for (int32 i = 1; i <= NumBlue; ++i) BlueCards.Add(FString::Printf(TEXT("B_%d"), i));
	for (int32 i = 1; i <= NumYellow; ++i) YellowCards.Add(FString::Printf(TEXT("Y_%d"), i));
	for (int32 i = 1; i <= NumWhite; ++i) WhiteCards.Add(FString::Printf(TEXT("W_%d"), i));
	...
	for (int32 i = 0; i < TotalCardPacks; ++i)
	{
		...
		MyGameInstance->CardPackDataList.Add(FCardPackData(PackCards));
	}

	// 카드팩 생성
	SpawnCardPacksFromData();
}

void AMyCardPackManager::SpawnCardPacksFromData()
{
	...
	int32 TotalCardPacks = MyGameInstance->CardPackDataList.Num();
	...
	for (int32 i = 0; i < TotalCardPacks; ++i)
	{
		...
		if (CardPack)
		{
			...
			CardPack->SetCards(MyGameInstance->CardPackDataList[i].Cards);

			FString CardsString = FString::Join(MyGameInstance->CardPackDataList[i].Cards, TEXT(", "));
